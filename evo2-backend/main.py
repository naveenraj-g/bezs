...
evo2_image = (
    ...
    .run_commands(
        "sed -i 's/np.fromstring(text, dtype=np.uint8)/np.frombuffer(text.encode(), dtype=np.uint8)/' /evo2/vortex/vortex/model/tokenizer.py"
    )
...
def brca1_example():
    ...
    print("Running BRCA1 variant analysis wiuth Evo2...")
    ...
    if "plot" in result:
        ...
        with open("brcal_analysis_plot.png", "wb") as f:
            f.write(plot_data)
